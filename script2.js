...
function cardHTML(r) {
  ...
  return `
    <div class="card ${r.starred ? 'starred' : ''}" data-id="${r.id}" data-editing="false">
      ...
      ${r.richNotes ? '<span style="font-size:10px;margin-left:6px;color:var(--accent3);vertical-align:middle;" title="Has rich notes">📝 notes</span>' : ''}
      ...
      <div class="cf">
        ...
        <div class="ca">
          ...
          <div class="ib opb" onclick="window.open('${esc(r.url)}','_blank')" title="Open link">↗</div>
          <div class="ib edt-btn" onclick="openEditor(${r.id})" title="Open Notes Editor 📝" style="font-size:14px;">📝</div>
...
let currentEditorId   = null;   // resource id whose notes are open
let editorDirty       = false;  // unsaved changes flag
let editorAutoSaveTimer = null;

/* ── Open editor for a specific card ── */
function openEditor(id) {
  ...
  // Load saved rich content or plain notes
  const saved = r.richNotes || '';
  const body  = document.getElementById('editor-body');
  body.innerHTML = saved || (r.notes
    ? `<p>${escForEditor(r.notes)}</p>`
    : '');
...
/* ── Save editor content back to the resource ── */
function saveEditor() {
  if (!currentEditorId) return;
  const r = res.find(x => x.id === currentEditorId);
  if (!r) return;

  const body    = document.getElementById('editor-body');
  r.richNotes   = body.innerHTML;
  // Also update plain notes as a stripped fallback
  r.notes       = body.innerText.trim().slice(0, 200);
...
function onEditorInput() {
  ...
  editorAutoSaveTimer = setTimeout(() => {
    saveEditor();
    document.getElementById('em-autosave-status').textContent = '✓ Auto-saved';
  }, 2000);
  document.getElementById('em-autosave-status').textContent = '...editing';
}

function setBadge(saved) {
  const b = document.getElementById('em-badge');
  b.textContent = saved ? '✓ Saved' : '● Unsaved';
  b.className   = 'em-badge' + (saved ? ' saved' : '');
}
...
function editorKeydown(e) {
  if (e.ctrlKey || e.metaKey) {
    if (e.key === 's') { e.preventDefault(); saveEditor(); }
...
function updateWordCount() {
  ...
  document.getElementById('wc-words').textContent = words + ' words';
  document.getElementById('wc-chars').textContent = chars + ' chars';
  document.getElementById('wc-lines').textContent = lines + ' lines';
}

/* ── Tab switching ── */
function switchTab(name, el) {
  document.querySelectorAll('.em-tab').forEach(t => t.classList.remove('active'));
  document.querySelectorAll('.em-tab-content').forEach(t => t.classList.remove('active'));
  if (el) el.classList.add('active');
  document.getElementById('tab-' + name).classList.add('active');

  // On preview tab — sync HTML
  if (name === 'preview') {
    document.getElementById('preview-body').innerHTML =
      document.getElementById('editor-body').innerHTML ||
      '<p style="color:var(--muted);font-style:italic">Nothing written yet…</p>';
  }
}

/* ── PDF / TXT / MD file handling ── */
...
function loadFile(file) {
  ...
  document.getElementById('pdf-filename').textContent = '📄 ' + name;
  ...
  if (ext === 'pdf') {
    // For PDF: read as text (basic extraction — readable in modern browsers)
    reader.onload = function(ev) {
      // Try to extract readable text strings from the binary
      const raw     = ev.target.result;
      const strings = extractPdfText(raw);
      const preview = document.getElementById('pdf-preview-area');
      preview.textContent = strings || '⚠ Could not extract text from this PDF. Try a text-based PDF.';

      // Ask if user wants to send to editor
      if (strings && confirm('PDF text extracted! Load it into the editor?')) {
        const body = document.getElementById('editor-body');
        body.innerHTML += '<hr><h3>📄 From: ' + escForEditor(name) + '</h3>'
          + '<p>' + escForEditor(strings).replace(/\n{2,}/g, '</p><p>').replace(/\n/g, '<br>') + '</p>';
        switchTab('write', document.querySelector('.em-tab[data-tab="write"]'));
        onEditorInput();
      }
    };
    ...
    // TXT / MD — just read as text
    reader.onload = function(ev) {
      const text    = ev.target.result;
      const preview = document.getElementById('pdf-preview-area');
      preview.textContent = text;

      if (confirm('Load "' + name + '" into the editor?')) {
        const body = document.getElementById('editor-body');
        const formatted = ext === 'md'
          ? mdToHtml(text)
          : '<p>' + escForEditor(text).replace(/\n{2,}/g,'</p><p>').replace(/\n/g,'<br>') + '</p>';
        body.innerHTML += '<hr><h3>📄 From: ' + escForEditor(name) + '</h3>' + formatted;
        switchTab('write', document.querySelector('.em-tab[data-tab="write"]'));
        onEditorInput();
      }
    };
...
/* Basic PDF text extraction — pulls readable ASCII strings */
function extractPdfText(binary) {
  const results = [];
  // Match BT...ET blocks (PDF text objects)
  const btEt = binary.match(/BT[\s\S]*?ET/g) || [];
  btEt.forEach(block => {
    const tj = block.match(/\(([^)]{2,})\)\s*T[jJ]/g) || [];
    tj.forEach(m => {
      const inner = m.match(/\(([^)]+)\)/);
      if (inner) results.push(inner[1].replace(/\\[rn]/g, ' '));
    });
  });
  // Fallback: grab long printable strings
  if (!results.length) {
    const raw = binary.match(/[^\x00-\x1F\x7F-\xFF]{4,}/g) || [];
    return raw.filter(s => s.trim().length > 3).slice(0, 300).join('\n');
  }
...
function downloadHTML() {
  ...
  const r       = res.find(x => x.id === currentEditorId);
  const title   = r ? r.title : 'StudyVault Notes';
  const html    = `<!DOCTYPE html><html><head><meta charset="UTF-8">
<title>${escForEditor(title)}</title>
<style>
  body { font-family: 'Segoe UI', sans-serif; max-width: 800px; margin: 40px auto; padding: 0 20px; line-height: 1.7; color: #222; }
  h1,h2,h3 { color: #4a7c59; }
  blockquote { border-left: 4px solid #4a7c59; padding: 8px 16px; color: #666; font-style: italic; }
  a { color: #4a7c59; }
</style></head><body>
<h1>${escForEditor(title)}</h1>
${content}
</body></html>`;
  downloadBlob(html, (title || 'notes') + '.html', 'text/html');
}

function downloadTXT() {
  const text  = document.getElementById('editor-body').innerText;
  const r     = res.find(x => x.id === currentEditorId);
  const title = r ? r.title : 'notes';
...
<style>
  body { font-family: 'Segoe UI', sans-serif; max-width: 800px; margin: 40px auto; line-height: 1.7; }
  h1,h2,h3 { color: #4a7c59; }
  blockquote { border-left: 4px solid #4a7c59; padding: 8px 16px; color: #666; font-style: italic; }
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape' && document.getElementById('editor-overlay').classList.contains('open')) {
    closeEditor();
  }
});
